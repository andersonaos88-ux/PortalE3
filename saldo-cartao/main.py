...
async def keep_alive():
    """Pinga o próprio serviço a cada 5 minutos para evitar inatividade."""
    await asyncio.sleep(60)  # aguarda 1 min após o start
    while True:
        try:
            async with httpx.AsyncClient() as client:
                await client.get(f"http://localhost:{PORT}/", timeout=10)
            print("[keep-alive] ping ok")
        except Exception as e:
            print(f"[keep-alive] erro: {e}")
        await asyncio.sleep(5 * 60)  # pinga a cada 5 minutos
...
app = FastAPI(title="API Saldo Cartão - Apoio ao Cliente", lifespan=lifespan)
